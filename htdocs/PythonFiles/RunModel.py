from ModelCalculations import calculateMeanBetweenKeyTimes, calculatePressTimes, calculateTimeBetweenKeys, calculateMeanPressTimes, calculateVectorDifference
...
def generateDataFrames(json_file):
    ...
    #keyDowntimes = getKeyDownTimers(keydown_data, keyup_data)
    timebetweenKeys = []
...
def getUniqueUsernames(excel_file, column_name='Username'):
    try:
        # Read the Excel file
        df = pd.read_excel(excel_file)
        
        # Extract unique usernames from the specified column
        unique_usernames = df[column_name].unique().tolist()
...
def storeConfidenceIntervalsInJSON(username, betweenKeysInterval, keyPressInterval, json_file="resources/ConfidenceIntervals.json"):
    try:
        # Read existing JSON data if the file is not empty
        with open(json_file, 'r') as f:
            data = json.load(f)
    ...
    # Update or add the confidence intervals for the specified user
...
def loginSequence(df_result_tuple):
     ...
     if userEntries > 9 and PASSWORD == CONST_PASSWORD:
        ...
        for user in getUniqueUsernames('resources/BetweenKeysData.xlsx'): # going through all the users in the database to see which one is the closest to the sample
            ...
            if presstime_vector_difference * betweenkey_vector_difference < min_difference:
        ...
        if login_result: # if the login was succesful we also store the data in the excel files
            ...
            storeData(pd.DataFrame(KEYPRESSTIMES, index=[0]), 'resources/KeyPressData.xlsx')
            storeData(pd.DataFrame(BETWEENKEYSTIMES, index=[0]), 'resources/BetweenKeysData.xlsx')
